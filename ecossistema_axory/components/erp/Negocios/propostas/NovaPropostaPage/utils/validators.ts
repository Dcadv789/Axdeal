...
type TipoDocumento = 'proposta' | 'venda' | 'os';
...
export const validarFormularioProposta = (
  ...
  // Validar número da proposta
  if (!formData.numeroProposta.trim()) {
    return {
      valido: false,
      mensagem: 'Por favor, preencha o número da proposta'
    };
  }
  ...
  // Validar itens individuais (código obrigatório apenas para proposta; venda/OS podem ter itens sem código)
  for (let i = 0; i < items.length; i++) {
    const item = items[i];

    if (tipo === 'proposta' && !item.codigo.trim()) {
      return {
        valido: false,
