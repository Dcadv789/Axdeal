...
import CatalogoDetailsDrawer from './catalogo/CatalogoDetailsDrawer';
import CatalogoDuplicateModal from './catalogo/CatalogoDuplicateModal';
import EmailDrawer from './contatos/EmailDrawer';
import SearchBar from '../Negocios/propostas/components/SearchBar';
import { CADASTROS_TAB_LABELS, type CadastrosTab } from './shared/navigation';
...
interface CadastrosContentProps {
  ...
  onEditarContato: (id: string) => void;
  onVisualizarContato: (id: string) => void;
...
function CadastrosTable({
  ...
  const isContatos = activeTab === 'contatos';
  const rows = isContatos ? clientes : catalogoItems;
  ...
  const getSortIcon = (column: CatalogSortColumn | ClienteSortColumn) => {
    const currentColumn = isContatos ? clientesSortColumn : catalogoSortColumn;
  ...
  if (!loading && rows.length === 0) {
    return (
      <div className="text-center py-12 text-gray-500 dark:text-gray-400">
        {isContatos ? 'Nenhum contato encontrado.' : `Nenhum ${activeTab === 'produtos' ? 'produto' : 'serviço'} encontrado.`}
      </div>
    );
  }
              ...
              ? clientes.map((cliente) => (
                  <tr key={cliente.id} className="hover:bg-gray-50 dark:hover:bg-neutral-800 transition-colors">
                    ...
                    <td className="min-w-0 px-5 py-3">
                      <button onClick={() => onVisualizarContato(cliente.id)} className="block w-full min-w-0 text-left">
                    ...
                    <td className="w-[140px] px-5 py-3">
                      <div className="flex items-center justify-center gap-1">
                        <button
                          ...
                          onClick={() => onVisualizarContato(cliente.id)}
                        ...
                        <button
                          ...
                          title="Editar"
                          aria-label="Editar"
                          onClick={() => onEditarContato(cliente.id)}
...
export default function CadastrosContent({
  ...
  useEffect(() => {
    if (activeTab === 'contatos' || !user) return;
  ...
  useEffect(() => {
    ...
    const fetchClientes = async () => {
      if (!cadastrosCache.clientesHydrated && activeTab === 'contatos') {
    ...
    <div className="space-y-6 py-6">
      <div className="relative space-y-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{`Cadastros > ${CADASTROS_TAB_LABELS[activeTab]}`}</h1>
        </div>
      ...
      <Card>
                ...
                {activeTab === 'contatos' ? (
                  <>
                    ...
                    <FilterSelect value={clientesTipo} onChange={(value) => setClientesTipo(value as 'todos' | 'PF' | 'PJ')}>
                      ...
                      <option value="PF">Contato PF</option>
                      <option value="PJ">Contato PJ</option>
                    </FilterSelect>
                  </>
                ) : (
        ...
        <CadastrosTable
          ...
          loading={activeTab === 'contatos' ? loadingClientes && !cadastrosCache.clientesHydrated : loadingCatalogo}
          catalogoSortColumn={sortColumn}
          clientesSortColumn={clientesSortColumn}
          sortDirection={activeTab === 'contatos' ? clientesSortDirection : sortDirection}
          ...
          onVisualizarContato={onVisualizarContato}
          onEditarContato={onEditarContato}
        />
      </Card>
