...
COMMENT ON COLUMN public.crm_leads.valor_estimado_contrato IS
'Valor estimado do contrato associado ao lead para projeção de pipeline.';

CREATE INDEX IF NOT EXISTS idx_crm_leads_valor_estimado_contrato
  ON public.crm_leads(valor_estimado_contrato);
...
'View consolidada de leads para relatórios CRM (lead + quiz + pipeline + etapa + UTM + status de negócio + data de fechamento + motivo da perda + valor estimado + agregados de respostas).';
