...
SET status_negocio = 'aberto'
WHERE status_negocio IS NULL
   OR btrim(status_negocio) = '';
...
COMMENT ON COLUMN public.crm_leads.status_negocio IS
'Status final do negocio: aberto, ganho ou perdido.';

COMMENT ON VIEW public.crm_vw_relatorio_leads IS
'View consolidada de leads para relatorios CRM (lead + quiz + pipeline + etapa + UTM + status de negocio + agregados de respostas).';
